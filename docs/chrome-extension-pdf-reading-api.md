...
- **`chrome.sidePanel.open()` 只能在用户手势响应中调用**（例如：点击工具栏扩展图标、侧栏内按钮、快捷键、右键菜单等）。  
  在 **`tabs.onUpdated` 等后台事件**中直接调用 `open()` 会失败（错误信息类似：`sidePanel.open() may only be called in response to a user gesture`）。
- **推荐做法**：后台仅做 `setOptions`（注册/启用当前标签页的侧栏配置）；**真正「打开」侧栏**使用 `setPanelBehavior({ openPanelOnActionClick: true })`（工具栏图标）、侧栏/内容脚本内的用户点击，或快捷键、右键菜单等。
...
flowchart LR
  user[UserGesture]
  action[ToolbarActionClick]
  setOpts[sidePanel.setOptions]
  panel[SidePanelOpens]
  user --> action
  action --> panel
  setOpts --> panel
```
...
| 说明 | 路径 |
| --- | --- |
| Manifest 合并：`action.default_title`、`side_panel`、`permissions`（`sidePanel`、`scripting`）、`host_permissions` | [package.json](../package.json) 中 `manifest` 字段 |
| Service worker：`setPanelBehavior({ openPanelOnActionClick: true })`、按 PDF URL 调用 `sidePanel.setOptions` | [background.ts](../background.ts) |
| 主界面侧栏 UI（summary / translation / highlight / comment） | [sidepanel.tsx](../sidepanel.tsx) |
